...
exports.addCourse = asyncHandler(async (req, res, next) => {
  //Reffer to model-course-bootcamp
  req.body.bootcamp = req.params.bootcampId;

  req.body.user = req.user.id;

  const bootcamp = await Boocamp.findById(req.params.bootcampId);
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      //customised error response
      new ErrorResponse(`User ${req.user.id} is not authorized to update course ${course_.id} to bootcamp${bootcamp._id}`, 401)
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      //customised error response
      new ErrorResponse(`User ${req.user.id} is not authorized to delete course ${course_.id} to bootcamp${bootcamp._id}`, 401)
    );
  }
